...
import statistics
import torch
from assets.py_vad_tool.audio_tools import read_wav, enframe, deframe
from assets.py_vad_tool.unsupervised_vad import nrg_vad
...
def create_UASpeech_custom_v2():
    ...
    wordlist = []
    with open(hparams.uaspeech_sagi_word_file, newline="") as handle:
    ...
    # filling a dictionary where speaker is key,
    # with one valid file for each of the 765 utterances,
    # where the mic is chosen randomly
    results = {}
    _, dirs, _ = next(os.walk(hparams.uaspeech_sagi_audio_dir))
    for dir in sorted(dirs):
        speaker = dir
        print(f"Finding valid files for speaker {speaker} ... ")
        valid_files = []
        _, _, files = next(os.walk(os.path.join(hparams.uaspeech_sagi_audio_dir, dir)))
    ...
    # delete 3 files from every speaker (765 - 3 = 762)
    # because for patient M01, those files are 30+ seconds long
    # and contain only background noise pretty much
    # delete 1 file from every speaker as well,
    # from patient M10, since just blabbering with somebody
    print("Removing stuff from speakers")
    ...
    for speaker in results:
        ...
        if not os.path.exists(
            os.path.join(hparams.uaspeech_custom_3_audio_dir, speaker)
        ):
            os.makedirs(os.path.join(hparams.uaspeech_custom_3_audio_dir, speaker))
        for file in results[speaker]:
            file_name = file.split("/")[-1]
            src = file
            dest = os.path.join(hparams.uaspeech_custom_3_audio_dir, speaker, file_name)
            copyfile(src, dest)
...
def cut_vad(c, nrg):
    ...
    """
    print(f"Starting cut_vad({c},{nrg})")
        ...
        for file in sorted(files):
            if ".wav" in file:
                ...
                meta_path = os.path.join(
                    hparams.uaspeech_custom_3_inference_dir, dir, file
                )
                ...
                if not os.path.exists(meta_path):
                    ...
                    np.save(
                        os.path.join(meta_path, file + "_mspecOutNoT.npy"), g_mel_noT
                    )
...
def scatter_plot_regression():
    ...
    meta = (
        "reference_2"  # meta_0, meta_1, meta_2, meta_3
        #                and for all of then '_swapped'
    )
    ...
    for speaker in path_speaker_codes:
        x = np.average(
            random.sample(path_speaker_codes[speaker], samples)
        )  # look only at randomly chosen X values
    ...
    for speaker in control_speaker_codes:
        x = np.average(
            random.sample(control_speaker_codes[speaker], samples)
        )  # look only at randomly chosen X values
        y = intelligibility[speaker]  # intelligibility 1-100%
        # y = 0                                                 # healthy == 0
...
def nice_scatter(x_c, y_c, x_p, y_p, samples, avg_x, avg_x_p, m, b, m_p, b_p):
    ...
    """
    out_path = f"{out_dir}/scatter"
    times_font = {"fontname": "Times New Roman"}
    font = font_manager.FontProperties(
        family="Times New Roman", style="normal", size=14
    )

    plt.scatter(x_p, y_p, s=100, color="red", marker="x", label="pathological speaker")
    plt.scatter(
        x_c,
        y_c,
        s=100,
...
def dtw_diff_condense():
        ...
        for subdir in sorted(subdirs):
            ...
            for refdir in sorted(m):
                if refdir not in speaker_files:
                    print(
                        f"Mistake: {dir}/{subdir}/{refdir} should be there, but is not!"
                    )
    ...
    for dir in sorted(dirs):
        ...
        for subdir in sorted(subdirs):
            ...
            for filedir in sorted(filedirs):
                _, _, files = next(
                    os.walk(os.path.join(root_dir, dir, subdir, filedir))
                )
                tmp = filedir.split("_")[1:3]
                fid = "_".join(tmp)
                ...
                # ~ mspec
                mspec_file = [
                    file if file.split("_")[4] == "mspec.npy" else None
                    for file in sorted(files)
                ]
                mspec_file = next(item for item in mspec_file if item is not None)
                mspec_file_path = mspec_file
                mspec_file = np.load(
                    os.path.join(root_dir, dir, subdir, filedir, mspec_file)
                )
                mspec_ref = ref[3]
                # dtw
                ...
                mspec_dtw_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, mspec_dtw_file
                )
                mspec_diff_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, mspec_diff_file
                )
                mspec_ref_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, mspec_ref_file
                )
                ...
                # ~ mspecOutNoT
                mspecOutNoT_file = [
                    file if file.split("_")[4] == "mspecOutNoT.npy" else None
                    for file in sorted(files)
                ]
                mspecOutNoT_file = next(
                    item for item in mspecOutNoT_file if item is not None
                )
                mspecOutNoT_file_path = mspecOutNoT_file
                mspecOutNoT_file = np.load(
                    os.path.join(root_dir, dir, subdir, filedir, mspecOutNoT_file)
                )
                mspecOutNoT_ref = ref[4]
                # dtw
                mspecOutNoT_dist, mspecOutNoT_path = dtw(
                    mspecOutNoT_ref, mspecOutNoT_file
                )
                mspecOutNoT_dtw = np.empty(
                    shape=(mspecOutNoT_ref.shape), dtype=np.float32
                )
                ...
                mspecOutNoT_dtw_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, mspecOutNoT_dtw_file
                )
                mspecOutNoT_diff_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, mspecOutNoT_diff_file
                )
                mspecOutNoT_ref_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, mspecOutNoT_ref_file
                )
                ...
                # ~ Zc
                zc_file = [
                    file if file.split("_")[4] == "Zc.npy" else None
                    for file in sorted(files)
                ]
                ...
                zc_dtw_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zc_dtw_file
                )
                zc_diff_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zc_diff_file
                )
                zc_ref_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zc_ref_file
                )
                zc_diff_avg_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zc_diff_avg_file
                )
                ...
                # ~ Zf
                zf_file = [
                    file if file.split("_")[4] == "Zf.npy" else None
                    for file in sorted(files)
                ]
                ...
                zf_dtw_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zf_dtw_file
                )
                zf_diff_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zf_diff_file
                )
                zf_ref_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zf_ref_file
                )
                zf_diff_avg_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zf_diff_avg_file
                )
                ...
                # ~ Zr
                zr_file = [
                    file if file.split("_")[4] == "Zr.npy" else None
                    for file in sorted(files)
                ]
                ...
                zr_dtw_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zr_dtw_file
                )
                zr_diff_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zr_diff_file
                )
                zr_ref_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zr_ref_file
                )
                zr_diff_avg_file_path = os.path.join(
                    root_dir, dir, subdir, filedir, zr_diff_avg_file
                )
            ...
            for filedir in sorted(filedirs):
                ...
                for file in sorted(files):
                    if "npy" in file:
                        if "diff" in file:
                            os.remove(
                                os.path.join(root_dir, dir, subdir, filedir, file)
                            )
                        if "dtw" in file:
                            os.remove(
                                os.path.join(root_dir, dir, subdir, filedir, file)
...
def n_utterances_t_times(n, T):
    ...
    version = "UASPEECH_Custom_2"
    meta = (
        "reference_2"  # meta_0, meta_1, meta_2, meta_3 - and for all of then '_swapped'
    )
...
def plot_everything(plot_dir):
    ...
    mspecOutNoT_diff = np.load(
        os.path.join(plot_dir, file_id + "_mspecOutNoT_diff.npy")
    )
    ...
    with torch.no_grad():
        fig, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(
            5, 4, constrained_layout=True, sharey="row"
        )
        # mspec
